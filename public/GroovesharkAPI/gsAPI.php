...
class gsAPI{
	...
	*/
	...
	/*
	* -----added by RO -----------
	* Returns a tinysongURL from the SongID		
	...
	@param	int	songID
	*/
	public static function getTinysongURLFromSongID($songID){		
		if (!is_numeric($songID)){
			trigger_error(__FUNCTION__." requires a valid songID.",E_USER_ERROR);
			return false;
		}
		
		$return = self::apiCall('getTinysongURLFromSongID',array('songID'=>$songID));
		if (isset($return['decoded']['result']['url']))
			return $return['decoded']['result']['url'];
		else {
		    gsAPI::$lastError = $return['raw'];
